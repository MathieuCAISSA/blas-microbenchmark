...
static void *setup(size_t dim1, size_t dim2, unsigned int thread_count)
{
    ...
    for (i = 0; i < m; i++) {
        ...
        for (j = 0; j < m; j++) {
            h_a[j * m + i] = (i == j) ? 2.0 : (0.5 / (double) m);
        }
    }
    for (i = 0; i < m * n; i++) {
        ctx->h_b0[i] = (double) (i % 100) * 0.01 - 0.5;
    }
...
static void call(void *vctx)
{
    ...
    bmb_gpu_dtrmm(ctx->m, ctx->n, ctx->alpha, ctx->d_a, ctx->m, ctx->d_b, ctx->m);
    bmb_gpu_synchronize();
}
