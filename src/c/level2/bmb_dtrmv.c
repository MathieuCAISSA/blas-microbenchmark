...
static void *setup(size_t dim1, size_t dim2, unsigned int thread_count)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++) {
            /* Diagonally dominant so repeated multiplication stays well
             * scaled even though X is reset before every call anyway. */
            ctx->a[i * n + j] = (i == j) ? 2.0 : (0.5 / (double) n);
        }
        ctx->x0[i] = (double) (i % 100) * 0.01 - 0.5;
    }
...
static void call(void *vctx)
{
    ...
    cblas_dtrmv(CblasRowMajor, CblasUpper, CblasNoTrans, CblasNonUnit, ctx->n, ctx->a, ctx->n, ctx->x, 1);
}
