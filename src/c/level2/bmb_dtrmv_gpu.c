...
static void *setup(size_t dim1, size_t dim2, unsigned int thread_count)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++) {
            /* Diagonally dominant so repeated multiplication stays well scaled. */
            h_a[j * n + i] = (i == j) ? 2.0 : (0.5 / (double) n);
        }
        ctx->h_x0[i] = (double) (i % 100) * 0.01 - 0.5;
    }
...
static void call(void *vctx)
{
    ...
    bmb_gpu_dtrmv(ctx->n, ctx->d_a, ctx->n, ctx->d_x, 1);
    bmb_gpu_synchronize();
}
