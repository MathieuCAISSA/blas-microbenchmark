#include <stdlib.h>

#include "bmb_bench.h"
#include "bmb_gpu.h"
#include "bmb_log.h"

/* Square routine: only dim1 (= N) is used, swept via --matrix-dim1. */
typedef struct {
    int n;
    double alpha;
    double beta;
    double *d_a; /* N x N, column-major, upper triangle used */
    double *d_x;
    double *d_y;
} bmb_ctx_t;

static void *setup(size_t dim1, size_t dim2, unsigned int thread_count)
{
    bmb_ctx_t *ctx;
    size_t n = dim1;
    double *h_a;
    double *h_x;
    double *h_y;
    size_t i;

    (void) dim2;
    (void) thread_count;

    ctx = malloc(sizeof(*ctx));
    if (ctx == NULL) {
        return NULL;
    }

    ctx->n = (int) n;
    ctx->alpha = 1.0;
    ctx->beta = 0.0;

    h_a = malloc(n * n * sizeof(double));
    h_x = malloc(n * sizeof(double));
    h_y = malloc(n * sizeof(double));
    if (h_a == NULL || h_x == NULL || h_y == NULL) {
        free(h_a);
        free(h_x);
        free(h_y);
        free(ctx);
        return NULL;
    }
    for (i = 0; i < n * n; i++) {
        h_a[i] = (double) (i % 100) * 0.01 - 0.5;
    }
    for (i = 0; i < n; i++) {
        h_x[i] = (double) (i % 100) * 0.01 - 0.5;
        h_y[i] = 0.0;
    }

    ctx->d_a = bmb_gpu_malloc(n * n * sizeof(double));
    ctx->d_x = bmb_gpu_malloc(n * sizeof(double));
    ctx->d_y = bmb_gpu_malloc(n * sizeof(double));
    bmb_gpu_memcpy_h2d(ctx->d_a, h_a, n * n * sizeof(double));
    bmb_gpu_memcpy_h2d(ctx->d_x, h_x, n * sizeof(double));
    bmb_gpu_memcpy_h2d(ctx->d_y, h_y, n * sizeof(double));
    free(h_a);
    free(h_x);
    free(h_y);

    return ctx;
}

static void call(void *vctx)
{
    bmb_ctx_t *ctx = vctx;

    bmb_gpu_dsymv(ctx->n, ctx->alpha, ctx->d_a, ctx->n, ctx->d_x, 1, ctx->beta, ctx->d_y, 1);
    bmb_gpu_synchronize();
}

static void teardown(void *vctx)
{
    bmb_ctx_t *ctx = vctx;

    bmb_gpu_free(ctx->d_a);
    bmb_gpu_free(ctx->d_x);
    bmb_gpu_free(ctx->d_y);
    free(ctx);
}

int main(int argc, char *argv[])
{
    bmb_benchmark_t bench = {0};
    bmb_gpu_status_t gpu_status;
    int status;

    gpu_status = bmb_gpu_init();
    if (gpu_status == BMB_GPU_NO_DEVICE) {
        bmb_log_warning("No compatible GPU device found; skipping.");
        return 77;
    }
    if (gpu_status != BMB_GPU_OK) {
        bmb_log_error("GPU backend initialization failed.");
        return EXIT_FAILURE;
    }

    bench.routine_name = "dsymv_gpu";
    bench.dim1_label = "Matrix dim (N)";
    bench.dim2_label = NULL;
    bench.use_vector_range = 0;
    bench.setup = setup;
    bench.call = call;
    bench.teardown = teardown;

    status = bmb_benchmark_main(argc, argv, &bench);
    bmb_gpu_shutdown();
    return status;
}
